...
![端到端产品工作流泳道图](../assets/diagrams/05_product_workflow_swimlane.svg)

当前MVP以责任规划师选择分析区域、高温情景与关注对象作为起点。专业模型负责构建空间事实、模拟活动路径、计算热暴露和生成干预情景；LLM Agent读取结构化事实，在允许动作和固定输出结构内解释活动失效、转译设施需求并组织规划行动卡；验证机制贯穿数据、路径、LLM输出与干预效果；规划师负责审议结果、补充现场信息并决定是否进入实施讨论。

| 关键交接点 | 上游输出 | 下游任务 | 产品控制机制 |
|---|---|---|---|
| 规划师 → Agent | 区域、高温情景、关注人群与活动 | 组织分析任务与工具调用目标 | 输入字段校验、缺失信息提示 |
| 专业模型 → LLM | 已核验的地点、路径、热暴露、活动约束与允许动作 | 解释失效原因、转译设施需求 | 只读结构化上下文、动作白名单 |
| LLM → 规则系统 | 结构化行为选择、原因解释与需求建议 | 检查事实引用、动作与输出格式 | JSON Schema、规则复核、异常回退 |
| 干预模型 → LLM | 设施情景、服务覆盖、暴露变化与实施限制 | 组织候选方案解释与行动卡 | 区分事实、推断、不确定性 |
| Agent → 规划师 | 问题卡、证据包、候选方案与待核验项 | 审议优先级并补充现场信息 | 人工确认、保留修改记录 |

### 5.3 未来规划Copilot用户旅程

下图展示居民自然语言诉求转译与规划行动卡生成的目标体验，属于后续版本设计。

![未来规划Copilot用户旅程](../assets/diagrams/03_product_workflow.svg)
